...
def make_interp(model, tar_l, temperature=1.):
    ...
    with torch.no_grad():
        ...
        z_0, z_1 = torch.randn((1, model.w_model.hparams.z_dim)) * temperature, torch.randn(
            (1, model.w_model.hparams.z_dim)) * temperature
        alpha = np.linspace(0, 1, num=n_grains, endpoint=True)
        z_interp = torch.cat([z_0 * (1 - a) + z_1 * a for a in alpha], 0)
        audio_interp = model.w_model.decode(z_interp, n_grains=n_grains, ola_windows=ola_windows, ola_folder=ola_folder,
                                            ola_divisor=ola_divisor).squeeze(0).numpy()
    return audio_interp
...
def generate_sample_for_class(model, class_name="HiHat", temperature=1.0):
    if class_name not in model.l_model.hparams.classes:
        raise ValueError(f"Class '{class_name}' not found in available classes: {model.l_model.hparams.classes}")

    cl_idx = model.l_model.hparams.classes.index(class_name)
...
def generate_loop(model, loop_len, drum_dict, temperature=1.):
    ...
    for cl in drum_dict:
        ...
        for onset, amplitude in events:
            audio = generate_sample_for_class(model, cl, temperature=temperature)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--action', choices=['interp', 'sample', 'loop'], required=True, help="Choose the action to perform: interp (interpolation), sample (new sound), or loop (generate a loop)")
    parser.add_argument('--class_name', default=None, type=str, help="The class name for generating a new sound. Required if action is 'sample'.")
    ...
    if args.finetuned:
        ...
        model = hierarchical_model(w_ckpt_file=w_ckpt_file, w_yaml_file=w_yaml_file, l_ckpt_file=l_ckpt_file,
    ...
    if args.action == 'interp':
        ...
        sf.write(os.path.join(args.export_dir, args.mname + "__sample" + str(args.samples_id) + "_interpolation.wav"),
                 audio_interp, model.w_model.hparams.sr)

    elif args.action == 'sample':
        if args.class_name is None:
            raise ValueError("A class name must be provided when the action is 'sample'.")

        print(f"\n*** generating a new sound for class {args.class_name}")
        audio_sample = generate_sample_for_class(model, args.class_name, temperature=args.temperature)
        audio_path = os.path.join(args.export_dir, f"{args.class_name}" + "_sample" + str(args.samples_id) + "_" + args.mname[:20] + ".wav")
        sf.write(audio_path, audio_sample, model.w_model.hparams.sr)
        print(f"File successfully writen to: {audio_path}")
        ...
        sf.write(os.path.join(args.export_dir, args.mname + "__sample" + str(args.samples_id) + "_loop.wav"), output_loop,
                 model.w_model.hparams.sr)
        for cl in drum_dict:
            sf.write(os.path.join(args.export_dir, args.mname + "__sample" + str(args.samples_id) + "_loop_" + cl + ".wav"),
                     loop_dict[cl], model.w_model.hparams.sr)
